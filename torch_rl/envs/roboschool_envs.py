...
path_to_roboschool = os.path.abspath(os.path.dirname(roboschool.__file__))
print(path_to_roboschool)
class RoboschoolReacher(RoboschoolMujocoXmlEnv):
    def __init__(self):
        RoboschoolMujocoXmlEnv.__init__(self, os.path.join(path_to_roboschool, 'mujoco_assets/reacher.xml'), 
            'body0', action_dim=2, obs_dim=9)
    ...
    def calc_state(self):
        ...
        return np.array([
            target_x,
            target_y,
            self.fingertip.pose().xyz()[0],
            self.fingertip.pose().xyz()[1],
            np.cos(theta),
            np.sin(theta),
            self.theta_dot,
            self.gamma,
    ...
    def calc_potential(self):
        return -10 * np.linalg.norm(self.to_target_vec)
    ...
    def _step(self, a):
        ...
        return state, self.potential, False, {}
